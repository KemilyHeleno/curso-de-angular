...
Encapsula lógicas dentro do mesmo componente, quando elas forem utilizadas juntas (sejam elas iguais ou diferentes), cada tipo de componente precisa de um service diferente, ou seja, as lógicas que estarão dentro do service, dependem da necessidade do componente.  
Services também são singletons dentro do escopo de um injector.  

Criação de um service na pasta product:  
`ng g s services/product`  

Chamada de um service dentro do Ts:  
`@Injectable({                  `  
`    providedIn: "root",        `  
`})                             `  
